...
class interval(object):
    def __init__(self):
        ...
        self.queuing_delay_optimal_mshr=[]
...
#def multi_RR(nonoverlap_insts,warp_numbers):
    def IPC_full(self):
        ...
        MSHR_delay_total_optimal_avl=0
        MSHR_delay_total_optimal_avl_mshr=0
        for i in self.queuing_delay_mshr:
            MSHR_delay_total_optimal_avl+=i
        for i in self.queuing_delay_optimal_mshr:
            MSHR_delay_total_optimal_avl_mshr+=i
        print(MSHR_delay_total_optimal_avl)
        print(MSHR_delay_total_optimal_avl_mshr)
        ipc=float(self.accum_inst)/(float(self.accum_inst)/self.max_issue+float(self.accum_stall)+float(self.nonoverlap_insts)/self.max_issue+float(MSHR_delay_total_optimal_avl))*self.warp_numbers
        if (ipc>self.max_issue):
            ipc=self.max_issue
        print(ipc)
        ...
        cycles=float(self.accum_inst)/self.max_issue+float(self.accum_stall)+float(self.nonoverlap_insts)/self.max_issue+float(MSHR_delay_total_optimal_avl)+float(DRAM_delay_total)
        ipc=float(self.accum_inst)*self.warp_numbers/float(cycles)
        print(ipc)
        print(self.total_instruction_counts)
        result=open('result_optimal_avl.txt','a')
        ...
        baseline_result=open('result_optimal_avl_mshr.txt','a')
        baseline_cycle=base_cycle+float(MSHR_delay_total_optimal_avl_mshr)+float(DRAM_delay_total)
        base_ipc=float(base_inst)/float(baseline_cycle)
        baseline_result.write(str(base_ipc)+','+str(self.total_instruction_counts)+'\n')
        ...
        for i in range(0,len(self.region_instructions[self.warp_index])/2):
            if (self.DRAM_READ_LIST[i] + self.DRAM_WRITE_LIST[i]) >= 1.0:
               ...
               # service_time = 128.0/self.dram_bandwidth
                ...
                utilization = float(service_time_sum) /float(self.region_instructions[self.warp_index][2*i]/self.max_issue+self.region_instructions[self.warp_index][2*i+1])
    ...
    def contention_modeling_MSHR_optimal_avl(self):                
        self.exp_reqs = 0
    ...
    def contention_modeling_MSHR_optimal_avl_mshr(self):                
        self.exp_reqs = 0
        for i in range(0,len(self.region_instructions[self.warp_index])/2):
            if(self.NoC_read_list[i]>=1):
                delay_mshr = 0
               ...
               # avg_stall_cycles=400
                ...
                self.exp_reqs =self.NoC_read_list[i] * self.warp_numbers
                if self.exp_reqs > self.numMshrs:                
                            delay_mshr = float(avg_stall_cycles * (math.ceil(float(self.exp_reqs) / float(self.numMshrs)) * 1.0))-float(avg_stall_cycles) 
                    # print "mshrs", self.numMshrs
                    # delay_mshr = (delay_mshr - self.avg_stall_cycles) * regionInfo["Read_Insts"]
                self.queuing_delay_optimal_mshr.append(delay_mshr)
                    #        delay_mshr+=self.queuing_delay_mshr[i]-(a-i-2)*self.avg_stall_cycles
                # print self.exp_reqs, self.numMshrs, delay_mshr
...
def main(kernel_id):
   ...
   # intervalInstance.contention_modeling_MSHR()
    ...
    intervalInstance.contention_modeling_MSHR_optimal_avl_mshr()
    #intervalInstance.contention_NoC_modeling()
    intervalInstance.contention_modeling_MSHR_optimal_avl()
