...
class interval(object):
    ...
    def calculate_avg_stall_cycles(self):
        ...
        self.avg_stall_cycles=200*(1-miss_rate)+300*miss_rate
    ...
    def dram_queue_length(self):
         for i in range(0,len(self.region_instructions[self.warp_index])/2):
             if(self.DRAM_READ_LIST[i]+self.DRAM_WRITE_LIST[i])>=1.0:
                ...
                if(batch>1):
                    total_request=(self.DRAM_READ_LIST[i]+self.DRAM_WRITE_LIST[i])*self.warp_numbers*self.activeCores/batch
                else:
                    total_request=(self.DRAM_READ_LIST[i]+self.DRAM_WRITE_LIST[i])*self.warp_numbers*self.activeCores
                print(total_request)
                dram_del=total_request*2
                self.queuing_delay_bank.append(dram_del)
             else:
                self.queuing_delay_bank.append(0)
        ...
        for i in range(0,len(self.region_instructions[self.warp_index])/2):
            if(self.NoC_read_list[i]>=1):
                ...
                if(self.DRAM_READ_LIST[i]>=1):
                    avg_stall_cycles=self.avg_stall_cycles+self.queuing_delay_bank[i]*self.L2_miss_rate
