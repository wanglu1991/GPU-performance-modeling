...
#def multi_RR(nonoverlap_insts,warp_numbers):
    def IPC_full(self):
        ...
        print(optimal_MSHR_delay_total)
        optimal_MSHR_delay_total_new=0
        for i in self.queuing_delay_mshr_optimal_new:
            optimal_MSHR_delay_total_new+=i
        print(optimal_MSHR_delay_total_new)
        ipc=float(self.accum_inst)/(float(self.accum_inst)/self.max_issue+float(self.accum_stall)+float(self.nonoverlap_insts)/self.max_issue+float(optimal_MSHR_delay_total))*self.warp_numbers
        if (ipc>self.max_issue):
            ipc=self.max_issue
        print(ipc)
        print("With Delay modeling")
       # DRAM_delay_total=0
       ...
       # print(DRAM_delay_total)
        ...
        cycles=float(self.accum_inst)/self.max_issue+float(self.accum_stall)+float(self.nonoverlap_insts)/self.max_issue+float(optimal_MSHR_delay_total)+float(NoC_delay_total)
        ipc=float(self.accum_inst)*self.warp_numbers/float(cycles)
        print(ipc)
        print(self.total_instruction_counts)
        result=open('result_delay_model+optimal_MSHR','a')
        result.write(str(ipc)+','+str(self.total_instruction_counts)+'\n')
        delay_result=open('result_delay_model','a')
        ...
        new_MSHR_result=open('result_delay_model+ceiling_MSHR','a')
